 ...
 *  https://www.programcreek.com/java-api-examples/?class=android.speech.SpeechRecognizer&method=createSpeechRecognizer
 */
public class RepeatActivity extends AppCompatActivity {
    @BindView(R.id.txtv_repeat_time)
    TextView txtv_duration;
    @BindView(R.id.txtv_repeat_sentence)
    TextView txtv_sentence;
    @BindView(R.id.txtv_repeat_speech)
    TextView txtv_speech;
    @BindView(R.id.imgview_mic)
    ImageView imageview_mic;
    @BindView(R.id.txtv_repeat_status)
    ...
    private void getSentences(){
        ...
        ArrayList<String> arrTemp =bundle.getStringArrayList(Constant.ARRAY_REPEAT_SENTENCES);
        if (arrTemp.size()>0){
            arrStringSentences = arrTemp;
        }else{
                        Cursor curSentences = MainActivity.database.getData("SELECT * FROM Sentences");
    ...
    public void nextSentence(){
        ...
        if(current_sentence_pos>total_sentences-1){
            ...
            intent.putExtra(Constant.TYPE_ACTION,Constant.TYPE_REPEAT);
    ...
    private boolean checkSpeechResult(String speech){
        if(speech.trim().toLowerCase().equals(current_sentence_content.trim().toLowerCase())){
            return true;
        }
        return false;
    }
